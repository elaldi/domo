...
function initDebug(debug_area_name) {
	debug_area = document.getElementById(debug_area_name);
	if (debug_area != null) {
		debug_area.innerHTML = "<h1>Debug area</h1>";
	}
}

function writeDebug(s, newLine = true) {
	if (debug_area != null) {
		debug_area.innerHTML += s;
		if (newLine) {
			debug_area.innerHTML += "<br/>";
		}
	}
}

function writeDebugSepa() {
	if (debug_area != null) {
		debug_area.innerHTML += "<br/><hr/>";
	}
}
...
function debug_dump (var_value, var_name)
{
	var cr = '<br/>';
	var fill = '&nbsp; &nbsp; &nbsp; &nbsp; ';
	var sepa = '.';
    ...
    var do_object = function(v)
    {
        ...
        if (v instanceof Array)
        {
            ...
            out = "Array("+num_elem+") \n" + cr +(indent.length === 0?'':sepa+indent+'')+"(";
            for (var i=0; i<num_elem; ++i)
            {
                out += "\n" + cr +(indent.length === 0?'':sepa+indent)+sepa+"   ["+i+"] = "+debug_dump(v[i],'',false,indent_by);
            }
            out += "\n" + cr +(indent.length === 0?'':sepa+indent+'')+")";
            return out;
        }
        else if (v instanceof Object)
        {
            ...
            out = "Object \n"+ (indent.length === 0?'':sepa+indent+'')+"(";
            for (var p in v)
            {
                out += "\n" + cr +(indent.length === 0?'':sepa+indent)+sepa+"   ["+p+"] = "+debug_dump(v[p],'',false,indent_by);
            }
            out += "\n" + cr +(indent.length === 0?'':sepa+indent+'')+")";
            return out;
        }
    ...
    switch (typeof var_value)
    {
        case "boolean":
            ...
            out += v_name + do_boolean(var_value) + cr;
            break;
        case "number":
            v_name = var_name.length > 0 ? var_name + ' = ':'';
            out += v_name + do_number(var_value) + cr;
            break;
        case "string":
            v_name = var_name.length > 0 ? var_name + ' = ':'';
            out += v_name + do_string(var_value) + cr;
            break;
        case "object":
            v_name = var_name.length > 0 ? var_name + ' => ':'';
        ...
        case "function":
            v_name = var_name.length > 0 ? var_name + ' = ':'';
            out += v_name + "Function" + cr;
            break;
        case "undefined":
            v_name = var_name.length > 0 ? var_name + ' = ':'';
            out += v_name + "Undefined" + cr;
            break;
        default:
            out += v_name + ' is unknown type!' + cr;
    }
